...
const PostLeft = ({ leftData, id }) => {
        ...
        if(data.is_like){
            try {
                const response = await fetch(`${BaseUrl}like_post`, {
                    ...
                    body: JSON.stringify({ token: JSON.parse(localStorage.getItem("loginInfo")).token, post_id: id })
                });
    ...
    return (
        <div className="flex flex-col w-full gap-4 mt-20">
            <div className="heart flex flex-col items-center gap-2" onClick={handleLikePost}>
                <FaHeartCirclePlus
                    size={30}
            ...
            <div className="bookmark flex flex-col items-center gap-2" onClick={handleSavePost}>
                {data?.is_save ? (
                    <FaBookmark size={30} />
                ) : (
                    <CiBookmark size={30} />
